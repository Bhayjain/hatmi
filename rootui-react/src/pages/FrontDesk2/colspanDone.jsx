...
class Content extends Component {
    constructor(props) {
        ...
        this.state = {
          weekDates:[],
          weekDatesNew:new Array(12).fill({}),
          activeAccordion:0,
          events: [
            {
              title: 'Event 1',
...
var end_date= [yearend, monthend, dayend].join('-');
    ...
    //   accordion_click()
      ...
      //     const isHighlighted = 
      //       (day.show_day === checkInDayName && hour >= check_in_time_hour) || // Highlight from check-in time onwards on the check-in day
      //       (day.show_day === checkOutDayName && hour <= check_out_time_hour) || // Highlight until check-out time on the check-out day
      //       (day.start_date > check_in_date_new && day.start_date < check_out_date); // Highlight all hours between check-in and check-out dates
              ...
              let cellContent;
                  ...
                  cellContent = (
                    <>
                      ...
                      <td className="show_new_td_data" colSpan={hours} style={{backgroundColor:room_details[j].status === "blocked" ? "#D0D3D9" : "#B6E9D1",
                      color:room_details[j].status === "blocked" ? "#989FAD" : "#12B76A"}}>
                          <div>{room_details[j].customer_name}</div>
                      </td>
                    </>
                  );
                    ...
                    <thead>
                          ...
                          {this.state.weekDates.map((val,ind)=>{
                            return(
                                  ...
                                  <th scope="col" colSpan={24} key={ind}>
                          ...
                          <tr>
                          {this.state.weekDatesNew.map((val,ind)=>{
                            return(
                           
                                  <th scope="col" colSpan={1} key={ind}>
                                    <div className="date_calendar">
                                        {val.show_date}
                                    </div>
                                    <div  className="day_calendar">
