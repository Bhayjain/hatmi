...
if(device_width < 600){
  var gk = (my_height/2) - 50;
   ...
   add_room = () => {
    ...
    if (this.state.aminities == "" || this.state.aminities == undefined || this.state.cost_type == "" || this.state.cost_type == undefined || this.state.icon_aminities == "" || this.state.icon_aminities == undefined ) {
        this.setState({
        ...
        var fd = new FormData();
          ...
          fd.append('cost', this.state.cost_type);
    
            console.log(...fd, "Add Room")
            const res = fetch(settings.api_url + "v1/master/room/create-room-aminitie",{
          method: 'POST',
          body: fd
      })
          .then((response) => response.json())
          .then(json => {
            console.log("Add Room Response**************************************", {response: json })
            var data = json;
            if (data.status == true) {
                this.setState({
                    profile_image :"",
...
for_edit_indoor=(val)=>{
    ...
    this.setState({
      modalOpen:true,
      heading_room:"Edit Room Amenity",
  ...
  edit_room = (room_id) => {
    ...
    if(files != undefined && files != '' && files != null){
      ...
      .then((response) => response.json())
      .then(json => {
        console.log("Edit Room Response**************************************", {response: json })
        var data = json;
        if (data.status == true) {
            this.setState({
                profile_image :"",
    ...
    delete_room=(room_id)=>{
        ...
        const res = fetch(settings.api_url + "v1/master/room/delete-room-aminitie/"+room_id, {
            method: 'Delete',
            headers: {
                "Content-type": "application/json; charset=UTF-8",
            }
                    ...
                    <Tabs.Content activeTab={ this.state.activeTab1 }>
                        ...
                        <Spinner color="primary" className="spinnerCss" style={{marginTop:gk,display: this.state.isLoading}}/>
                            <div className="salary_show test_collapse mycalendar" style={{display: this.state.isLoading=="none" ? "block" :"none", height : my_height-121}}>
                                ...
                                <h3 className="noDataMessage test_collapse" style={{ display: this.state.noDataFound_aminities, marginTop:gk}}>No Data Found</h3>
                                    <div className="paddingBoth" style={{display: this.state.noDataFound_aminities=="none" ? "block" :"none"}}>
                                            <div className="row">
                                                {this.state.room_array.map((val,index)=>{
                                                    return(
                                                        <div className="col-lg-2 col-md-2" style={{marginBottom:"30px"}} key={index}>
                                                            <div className="containerIconRoom">
                                                                <div className="img_icon roomaminities"><img  src={val.icon} alt="img" />

                                                                </div>
                                                                <div className="name_aminities">{val.name}</div>
                                                                {val.cost == "" || val.cost == undefined ? "" : <div className="costTypeShow">{val.cost}</div>}
                                                            </div>
                                ...
                                <div className="add_proprty test_collapse">
                                    <div className="test_collapse">
                                        <div className="row">
                                            ...
                                            <div>
                                            <input id="inputGroupFile01" type="file" placeholder="Choose Logo" accept="image/png, image/jpeg, image/jpg"  className="no_input_file" disabled={this.state.policy_dock_control == "false" ? 'disabled' : ''} multiple onChange={this.handleChangeFile_Quotationn} style={{display:"none"}} />
                                            ...
                                            <div className="col-lg-6 col-md-6 mb-15">
                                                   <Label className="labelforall">Cost<span className="start_mark">*</span></Label>
                                                  <div>
                                                  <div style={{ display: 'inline-flex' ,marginTop:"-8px"}}>
                                                      <CustomInput invalid={this.state.borderNew && this.state.cost_type == "" ? true : false } type="radio" id="formRadioOffer1" name="formRadio" label="Free"  checked={this.state.cost_type === "free"}
                                                          onClick={(e) => {
                                                              this.setState({
                                                                  cost_type: "free",
                                                                  error_meesage:""
                                                      ...
                                                      <CustomInput invalid={this.state.borderNew && this.state.cost_type == "" ? true : false } type="radio" id="formRadioOffer2" name="formRadio" label="Chargable"  checked={this.state.cost_type === "chargable"}
                                                          onClick={(e) =>{
                                                              this.setState({
                                                                  cost_type: "chargable",
                                                                  error_meesage:""
                                ...
                                <div className="add_proprty test_collapse">
                                    <div className="test_collapse">
                                        <div className="row">
                                            ...
                                            <div>
                                            <input id="inputGroupFile03" type="file" placeholder="Choose Logo" accept="image/png, image/jpg, image/jpeg"  className="no_input_file" disabled={this.state.policy_dock_control == "false" ? 'disabled' : ''} multiple onChange={this.handleChangeFileOutdoor} style={{display:"none"}} />
